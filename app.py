...
def send_reminder(reminder):
    try:
        ...
        to_number = reminder["to"]
        if not to_number.startswith("whatsapp:"):
            to_number = f"whatsapp:{to_number}"
        print(f"[DEBUG] Sending WhatsApp reminder to {to_number} from {TWILIO_WHATSAPP_NUMBER}: {reminder['msg']}")
        client.messages.create(
            body=f"⏰ Reminder: {reminder['msg']}",
            from_=TWILIO_WHATSAPP_NUMBER,
            to=to_number
        )
        print(f"Sent reminder to {to_number}: {reminder['msg']}")
...
if __name__ == "__main__":
    # Start the scheduler in the main thread if not already running
    if not scheduler.running:
        scheduler.start()
        print("Scheduler started in __main__")
